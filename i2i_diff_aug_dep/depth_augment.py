...
def depth_aware_augment(
    ...
    strength: float = 0.9,
    shift_magnitude: float = 55.0,
    # depth-mask controls
    gamma_over: float = 1.5,
    gamma_under: float = 2.0,
    depth_blend: float = 0.85,        # depth mask weight; 1-depth_blend = cluster mask
    ...
    core_threshold: float = 0.80,     # influence above which we force clip
) -> np.ndarray:
    ...
    if mode == "overexposed":
        ...
        if core.any():
            L_low_shifted[core] = (
                0.3 * L_low_shifted[core] + 0.7 * clip_target_over
            )
    else:  # underexposed
        # pull darker pixels harder — but crucially also make sure deep regions
        # get aggressive absolute darkening, not just proportional shift
        darkness_pull = np.power(1.0 - L_low / 100.0, 0.5)
        shift = exposure_map * shift_magnitude * (0.5 + 0.5 * darkness_pull)
        L_low_shifted = L_low - shift
        core = exposure_map > core_threshold
        if core.any():
            # Force hard black in the very deepest regions (fixes weak underexposure)
            L_low_shifted[core] = (
                0.2 * L_low_shifted[core] + 0.8 * clip_target_under
            )

    # ── 5. Recombine with high-frequency texture ──────────────────────────
    # NOTE: in very-dark core regions the high-frequency detail would produce
    # visible "wiggle" — we attenuate the high-pass there to get true blackness.
    # This is applied ONLY where the exposure map is strong.
    hf_attenuation = np.clip(1.0 - 0.7 * exposure_map, 0.0, 1.0)
