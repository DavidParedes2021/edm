...
# ─────────────────────────────────────────────────────────────────────────────
# Extreme-weighted L1 (mode='dark' or mode='bright')
# ─────────────────────────────────────────────────────────────────────────────
class ExtremeWeightedL1Loss(nn.Module):
    """Upweight pixel error where the TARGET is at an extreme of the L range.

    Counteracts the mean-regression bias of vanilla L1 — without this the
    underexposed model under-darkens deep cavities, and the overexposed
    model under-saturates near tissue.

    mode='dark'   — peak weight at target = -1 (pure black, L=0)
    mode='bright' — peak weight at target = +1 (pure white, L=100)

    Weight:  w = 1 + alpha * exp( -d(target) / tau )
    where d(target) = (target + 1) for dark, (1 - target) for bright,
    both clamped to [0, ∞) so out-of-range targets cannot blow up exp.
    """

    def __init__(self, mode: str = "dark", alpha: float = 5.0, tau: float = 0.3):
        super().__init__()
        if mode not in ("dark", "bright"):
            raise ValueError(f"mode must be 'dark' or 'bright', got {mode}")
        self.mode = mode
        self.alpha = alpha
        self.tau = tau
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "dark":
            shifted = (target_f + 1.0).clamp_min(0.0)   # 0 at -1, 2 at +1
        else:
            shifted = (1.0 - target_f).clamp_min(0.0)   # 0 at +1, 2 at -1
...
# Backwards-compat alias — old code imports DarknessWeightedL1Loss.
class DarknessWeightedL1Loss(ExtremeWeightedL1Loss):
    def __init__(self, alpha: float = 5.0, tau: float = 0.3):
        super().__init__(mode="dark", alpha=alpha, tau=tau)
